...
include('includes/connect.php');
	...
	if (isset($_POST['price_from'])) {
			...
			$wheref .= "price BETWEEN $price_from AND $price_to ORDER BY price";
	}
						

	$ProductList = mysqli_query($conn,"SELECT COUNT(id) as count FROM products WHERE $wheref");
	while($product = mysqli_fetch_array($ProductList))
	{
		$count = $product['count'];
			...
			$UserListN = mysqli_query($conn,"SELECT * FROM products WHERE $wheref");
